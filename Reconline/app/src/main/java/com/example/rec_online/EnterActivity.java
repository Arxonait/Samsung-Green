                                ...
                                try {
                                    ...
                                    if(answer.getBoolean("status")){
                                        ...
                                        if (locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
                                            ...
                                            if (ContextCompat.checkSelfPermission(EnterActivity.this, android.Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED
                                                    && ContextCompat.checkSelfPermission(EnterActivity.this, android.Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                                                // Запрос разрешений
                                                ActivityCompat.requestPermissions(EnterActivity.this, new String[]{android.Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION}, 1);
                                                process_load(true);
                                            } else {
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == 1) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED && grantResults[1] == PackageManager.PERMISSION_GRANTED) {
                // Разрешения предоставлены, переходим в активность MapsActivity
                Intent intent = new Intent(EnterActivity.this, MapsActivity.class);
                startActivity(intent);
                finish();
            } else {
                Toast.makeText(getApplicationContext(), "Предоставьте GPS",
                        Toast.LENGTH_SHORT).show();
            }
        }
    }
